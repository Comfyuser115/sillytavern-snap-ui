...
import { withSnapProfile } from "./connection.js";
...
// Fires a quiet (non-chat-visible) generation call via the "snap"
// connection profile and returns a fully formed Item (text message or
// snap — the model decides). Retries once with a stricter prompt on parse
// failure, then degrades to a plain text item rather than throwing.
export async function requestContactReply({ contactKey, contactName, personaText, recentContext, direction }) {
  ...
  setTyping(keyStr);
  try {
    return await withSnapProfile(async () => {
